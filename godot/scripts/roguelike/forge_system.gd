...
const SYMBOL_NAMES: Dictionary = {1: "검", 2: "활", 3: "지팡이", 4: "표창", 5: "방패", 6: "힐"}

static func get_symbol_name(symbol_id: int) -> String:
	if SYMBOL_NAMES.has(symbol_id): return str(SYMBOL_NAMES[symbol_id])
	return DiceData.name_for(symbol_id)

static func get_symbol_ids() -> Array[int]: return [1, 2, 3, 4, 5, 6]

static func get_symbol_ids_for_run(run_state: RunStateManager) -> Array[int]:
	if run_state == null: return get_symbol_ids()
	return LegacySystem.get_forge_options(run_state.selected_legacies)
...
static func modify_face(run_state: RunStateManager, die_index: int, face_index: int, symbol_id: int) -> Dictionary:
	if not can_modify(run_state, die_index, face_index): return {"success": false, "message": "대장간 사용 조건을 만족하지 않습니다."}
	if not get_symbol_ids_for_run(run_state).has(symbol_id): return {"success": false, "message": "현재 유산으로 선택할 수 없는 심볼입니다."}
	var success: bool = run_state.forge_change_face(die_index, face_index, symbol_id, FORGE_COST)
	if not success: return {"success": false, "message": "골드가 부족하거나 이미 대장간을 사용했습니다."}
...
static func describe_die(run_state: RunStateManager, die_index: int) -> Array[String]:
	...
	for face in run_state.get_die_faces(die_index): result.append(get_symbol_name(int(face)))
	return result

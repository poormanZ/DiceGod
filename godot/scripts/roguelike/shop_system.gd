...
const ITEMS: Array[Dictionary] = [
	{"id": "dice_reroll", "name": "주사위", "description": "보유 주사위 1개를 새 주사위로 교체", "cost": 80, "type": "dice"},
	{"id": "iron_armor", "name": "철갑 장비", "description": "몸통 / 최대 HP +15", "cost": 100, "type": "equipment"},
	{"id": "war_glove", "name": "전투 장갑", "description": "무기 / 공격력 +3", "cost": 120, "type": "equipment"},
	{"id": "lucky_coin", "name": "행운의 동전", "description": "반지 / 전투 골드 보상 강화", "cost": 140, "type": "equipment"}
]
...
static func purchase(run_state: RunStateManager, item_id: String) -> Dictionary:
	for item in ITEMS:
		...
		if item_id == "dice_reroll" and run_state.run_dice_faces.is_empty(): return {"success": false, "message": "주사위가 준비되지 않았습니다."}
...
static func _apply_item(run_state: RunStateManager, item_id: String) -> bool:
	...
	# 장비는 구매 시 보유 상태만 만들고, 실제 스탯/시너지는 상태 화면에서 장착할 때 적용한다.
	return true
